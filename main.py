...
keep_alive()
bot.run(DISCORD_TOKEN)                        user_inventory = {int(k): {int(uk): uv for uk, uv in v.items()} for k, v in data.get("user_inventory", {}).items()}
...
@tree.command(name="release", description="🗑️ Release a Pokémon to catch it again")
@app_commands.describe(pokemon="The name of the Pokémon to release")
async def release(interaction: discord.Interaction, pokemon: str):
    """Release a Pokémon from collection"""
    ...
    if not pokemon_list:
        await interaction.followup.send("❌ You don't have any Pokémon to release!", ephemeral=True)
        return
    ...
    if not target_pokemon:
        available_pokemon = ", ".join(sorted(set(p["name"] for p in pokemon_list)))
        await interaction.followup.send(f"❌ You don't have a **{pokemon_name}**!\n\n**Your Pokémon:** {available_pokemon}\n\nUse `/pokemon` to see your full collection.", ephemeral=True)
        return
    
    pokemon_list.pop(target_index)
    ...
    reward_map = {
        ...
        "uncommon": random.randint(100, 200),
        "rare": random.randint(200, 400),
        "legendary": random.randint(400, 800)
    }
    
    reward = reward_map.get(rarity, 100)
    ...
    embed = discord.Embed(
        ...
        description=f"You released **{pokemon_name}** back into the wild!\n\n{rarity_emoji[rarity]} **Rarity:** {rarity.title()}\n💰 **Reward:** {reward:,} coins\n\n✨ You can now catch this Pokémon again!",
        color=Colors.WARNING,
        timestamp=datetime.now(timezone.utc)
    )
    embed.add_field(name="Released Pokémon", value=f"{target_pokemon['name']}\n{target_pokemon['type']}\n❤️ HP: {target_pokemon['hp']} | ⚔️ ATK: {target_pokemon['atk']}", inline=True)
    embed.add_field(name="Your Balance", value=f"💰 **{coins + reward:,}** coins", inline=True)
    embed.add_field(name="Remaining Pokémon", value=f"📊 **{len(pokemon_list)}** total", inline=True)
    embed.set_thumbnail(url=get_pokemon_image(pokemon_data["id"]))
    embed.set_footer(text=f"CosmoCatch • {interaction.user.display_name}")
    
    await interaction.followup.send(embed=embed, ephemeral=True)
    asyncio.create_task(batch_save_database())

@tree.command(name="pokemon", description="📋 View your Pokémon collection")
async def pokemon_cmd(interaction: discord.Interaction, user: Optional[discord.Member] = None):
    ...
    embed.set_footer(text=f"Use /trade to exchange Pokémon!")
    
    await interaction.followup.send(embed=embed)

@tree.command(name="inventory", description="🎒 View your items and Pokémon")
...
@tree.command(name="battle", description="⚔️ Battle with Pokémon")
@app_commands.describe(
...
async def battle(interaction: discord.Interaction, mode: app_commands.Choice[str]):
    """Battle system - wild only for now"""
    await interaction.response.defer()
    ...
    max_hp = POKEMON_DATA[user_pokemon['name']]["hp"]
    current_hp = user_pokemon.get("current_hp", max_hp)
    
    if current_hp <= 0:
        await interaction.followup.send(f"❌ Your **{user_pokemon['name']}** has fainted! Use `/heal` to restore HP first.", ephemeral=True)
        return
    ...
    battle_start_embed = discord.Embed(
        ...
        description=f"🌿 **Wild {wild_name}** appeared!\n\n**Your {user_pokemon['name']}** vs **Wild {wild_name}**",
        color=Colors.WARNING,
        timestamp=datetime.now(timezone.utc)
    )
    battle_start_embed.set_thumbnail(url=get_pokemon_image(wild_data["id"]))
    battle_start_embed.set_footer(text="⏳ Battle starting...")
    
    message = await interaction.followup.send(embed=battle_start_embed)
    
    # Animated battle sequence - 7 seconds
    for i, frame in enumerate(BATTLE_FRAMES):
        battle_embed = discord.Embed(
            title="⚔️ BATTLE IN PROGRESS!",
            description=f"**{user_pokemon['name']}** vs **Wild {wild_name}**\n\n```{frame}```",
            color=Colors.DANGER,
            timestamp=datetime.now(timezone.utc)
        )
        battle_embed.set_thumbnail(url=get_pokemon_image(wild_data["id"]))
    ...
    while user_hp > 0 and wild_hp > 0 and turn <= 10:
    ...
    if user_hp > wild_hp:
        ...
        embed = discord.Embed(
            title="🏆 Victory!",
            description=f"Your **{user_pokemon['name']}** defeated the wild **{wild_pokemon['name']}**!\n\n💰 Reward: **{reward} coinsimport discord
...
BATTLE_FRAMES = [
    "⚔️  Pokémon are facing each other...",
    "💥  A fierce attack is launched!",
    "⚡  Energy crackling in the air!",
    "🔥  The battle intensifies!",
    "💫  Special moves are unleashed!",
    "⭐  The final blow is coming!",
    "✨  Battle reaching its climax!"
]

# Pokémon data with rarities, base stats, and images (Gen 1 - 135 Pokémon)
POKEMON_DATA = {
...
def get_balance(server_id: int, user_id: int) -> int:
    ...
    if user_id not in user_coins[server_id]:
        user_coins[server_id][user_id] = 1000
...
def get_coins(server_id: int, user_id: int) -> int:
    """Get user coins"""
    return get_balance(server_id, user_id)

def set_coins(server_id: int, user_id: int, amount: int):
    """Set user coins"""
    set_balance(server_id, user_id, amount)
