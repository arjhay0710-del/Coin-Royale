...
class Colors:
    ...
    DANGER = 0xED4245   # Red
    INFO = 0x5865F2     # Blue
    POKEMON = 0xFFCB05  # Pokémon Yellow
            ...
            if message.content.startswith("```json") and message.content.endswith("```"):
                try:
                    ...
                    if isinstance(data, dict) and "user_pokemon" in data:
                        ...
                        logging.info(f"✅ Successfully loaded Pokémon database")
                        return
...
async def save_database():
    ...
    try:
        ...
        embed = discord.Embed(
            title="💾 Pokémon Bot Database Backup",
...
async def on_ready():
    """Bot startup"""
    logging.info(f"🚀 Pokémon Bot logged in as {bot.user}")
    ...
    logging.info(f"🎮 Pokémon Bot is ready!")
