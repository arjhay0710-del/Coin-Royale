...
# Battle GIF URLs - Using direct CDN links
BATTLE_GIFS = {
    "wild": "https://media1.tenor.com/m/S2Z3bScPjJ0AAAAC/pokemon-battle.gif",
    "pvp": "https://media1.tenor.com/m/m7bER1iYmfAAAAAC/pokemon-battle.gif",
    "loading": "https://media1.tenor.com/m/tKFAQN4E-j8AAAAC/pokeball-pokemon.gif"
}
...
class PokemonSelectModal(discord.ui.Modal, title="Select Your Pokémon"):
    ...
    async def start_pvp_battle(self, interaction: discord.Interaction, challenger_pokemon: dict, opponent_pokemon: dict):
        ...
        prep_embed = discord.Embed(
            title="⚔️ Battle Challenge Accepted!",
        ...
        prep_embed.set_image(url=BATTLE_GIFS["loading"])
        prep_embed.set_footer(text="⏳ Preparing for battle...")
        
        message = await interaction.followup.send(embed=prep_embed)
        await asyncio.sleep(4)
        
        # Battle animation - use different embed to force refresh
        battle_embed = discord.Embed(
            title="⚔️ INTENSE BATTLE!",
            description=f"💥 **{challenger_pokemon['name']}** vs **{opponent_pokemon['name']}**\n\nThe Pokémon are giving it their all!",
            color=Colors.DANGER,
            timestamp=datetime.now(timezone.utc)
        )
        battle_embed.set_image(url=BATTLE_GIFS["pvp"])
        battle_embed.set_footer(text="⚔️ Battle in progress...")
